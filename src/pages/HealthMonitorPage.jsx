...
function computeProtocolHealthScore(vaults) {
  ...
  const avgStability =
    vaults.reduce((sum, v) => sum + (computeStabilityScore(v) ?? 0), 0) / vaults.length
  const avgTvl =
    vaults.reduce((sum, v) => sum + Number(v.analytics?.tvl?.usd || 0), 0) / vaults.length
...
function detectDecouplingRisks(vaults) {
  return vaults
    ...
    .map((v) => ({
      ...
      severity:
        Math.abs(v.analytics.apy.total / v.analytics.apy30d - 1) > 1 ? 'high' : 'medium',
    }))
    .slice(0, 5)
}
...
export default function HealthMonitorPage() {
  ...
  const protocolScores = Object.entries(byProtocol)
    .map(([name, pvaults]) => ({
      name,
                ...
                <div className="h-1.5 bg-surface-container rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        score > 0.75 ? '#009844' : score > 0.5 ? '#f59e0b' : '#ba1a1a',
                    }}
                  />
                </div>
              </div>
          ...
          <section className="col-span-12 lg:col-span-4 space-y-4">
            ...
            {decouplingRisks.length === 0 ? (
              <div className="bg-tertiary-container/10 border border-on-tertiary-container/20 p-6 rounded-xl text-center">
                ...
                <p className="font-bold text-on-tertiary-container mt-2">
                  No decoupling risks detected
                </p>
                <p className="text-xs text-on-surface-variant mt-1">
                  All monitored vaults within normal APY range
                </p>
              </div>
            ) : (
  ...
  return (
    <div className="animate-pulse space-y-4">
      ...
      <div className="grid grid-cols-12 gap-8 mt-6">
        {[4, 4, 4].map((cols, i) => (
          <div key={i} className={`col-span-4 space-y-3`}>
            {[1, 2, 3, 4, 5].map((j) => (
