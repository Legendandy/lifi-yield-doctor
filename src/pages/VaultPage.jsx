...
function computeUpgradeImpact(doctorsChoiceVault, allVaults, assumedBalanceUsd = 10_000) {
  ...
  // APY is already %, so divide by 100 for dollar math
  const annualGain = ((bestApy - medianApy) / 100) * assumedBalanceUsd
  if (annualGain < 1) return null

  return {
    apyBoost:  (bestApy - medianApy).toFixed(2),
    annualGain: annualGain.toFixed(0),
    medianApy:  medianApy.toFixed(2),
...
function DoctorsChoiceCard({ vault, impact, onDeposit, chainName }) {
  ...
  const apy    = vault.analytics.apy.total != null ? vault.analytics.apy.total.toFixed(2) : 'N/A'
  const apy30d = vault.analytics.apy30d != null ? vault.analytics.apy30d.toFixed(2) : null
    ...
    <div className="bg-surface-container-lowest rounded-2xl clinical-shadow overflow-hidden border border-surface-container">
      <div className="grid grid-cols-12 gap-0">
        <div className={`${impact ? 'col-span-12 lg:col-span-8' : 'col-span-12'} p-8 space-y-6`}>
          ...
          {!impact && (
            <button onClick={onDeposit} className="px-6 py-3 bg-primary-container text-white rounded-xl font-black text-sm hover:opacity-90 transition-colors">
              Deposit Now
            </button>
          )}
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 space-y-2">
                <div className="flex justify-between text-xs text-slate-400">
                  <span>Chain median APY</span><span>{impact.medianApy}%</span>
                </div>
                <div className="flex justify-between text-xs text-white font-bold">
                  <span>This vault APY</span><span className="text-tertiary-fixed">{impact.bestApy}%</span>
                </div>
              </div>
            </div>
            <button onClick={onDeposit} className="w-full mt-6 py-4 bg-white text-primary-container rounded-xl font-black text-sm hover:bg-slate-100 transition-colors">
              Execute Upgrade
            </button>
          </div>
        )}
...
function VaultTable({ vaults, allVaults, doctorsChoiceAddress, onDeposit, pageIndex, totalPages, totalVaults, onPageChange }) {
  const medianApy = (() => {
    ...
    <div className="bg-surface-container-lowest rounded-2xl clinical-shadow">
      ...
      <div className="divide-y divide-surface-container">
        {vaults.map((vault, i) => {
          ...
          // APY already a % — display directly
          const apy             = vault.analytics.apy.total != null ? vault.analytics.apy.total.toFixed(2) : null
          const apy30d          = vault.analytics.apy30d != null ? vault.analytics.apy30d.toFixed(2) : null
          ...
          const rankScore       = computeVaultRankScore(vault)
          const vsMedian        = apy && medianApy ? (Number(apy) - medianApy) : null
          ...
          if (vault.isTransactional === true && !isBestMatch) badges.push({ label: '⚡ Cross-chain deposit', color: 'bg-on-tertiary-container/10 text-on-tertiary-container' })
          if (vault.isTransactional === false) badges.push({ label: '🔒 Same-chain', color: 'bg-amber-100 text-amber-700' })
          if (vault.isRedeemable === false) badges.push({ label: '⚠ Not redeemable', color: 'bg-error/10 text-error' })

          return (
            <div key={vault.address + i}
              className={`px-6 py-4 grid grid-cols-12 gap-4 items-center hover:bg-surface-container-low transition-colors ${isBestMatch ? 'bg-tertiary-container/5' : ''}`}>
              ...
              <div className="col-span-4 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex gap-1 mt-1 flex-wrap">
                    ...
                    {badges.map((b, bi) => (
                      <span key={bi} className={`px-1.5 py-0.5 rounded text-[9px] font-black ${b.color}`}>{b.label}</span>
                    ))}
                  </div>
                </div>
              </div>
              <div className="col-span-2 text-right">
                ...
                {vsMedian !== null && vsMedian > 0 && (
                  <p className="text-[10px] font-bold text-on-tertiary-container mt-0.5">
                    +{vsMedian.toFixed(1)}% vs avg
                  </p>
                )}
              </div>
              <div className="col-span-1 text-right">
                <p className="text-sm font-medium text-on-surface-variant">{apy30d ? `${apy30d}%` : '—'}</p>
              </div>
              <div className="col-span-2 text-right">
                <p className="font-bold text-sm text-on-surface">{tvlDisplay}</p>
                <p className="text-[9px] text-on-surface-variant mt-0.5">Score {Math.round(rankScore * 100)}/100</p>
              </div>
