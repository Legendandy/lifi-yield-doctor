...
// REAL APY DRIFT: compute drift across all three timeframes
function computeApyDrift(vault) {
  ...
  return {
    ...
    absoluteDrift: Math.abs(apy.total - apy30d) + Math.abs(apy7d - apy30d) + Math.abs(apy1d - apy7d),
  }
}
...
export default function StabilityIndexPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setVaults(data)
      // Default select the vault with most drift data available
  ...
  // Vaults with full drift data for heatmap
  ...
  // Volatility tier for heatmap cell coloring
  function getVolatilityColor(drift) {
    const abs = Math.abs(drift?.absoluteDrift || 0)
    if (abs < 0.005) return { bg: '#dcfce7', text: '#166534' }    // Very stable
    if (abs < 0.015) return { bg: '#d1fae5', text: '#065f46' }    // Stable
    if (abs < 0.03)  return { bg: '#fef9c3', text: '#854d0e' }    // Moderate
    if (abs < 0.05)  return { bg: '#fed7aa', text: '#9a3412' }    // Drifting
    return { bg: '#fecdd3', text: '#9f1239' }                      // Volatile
  }
  ...
  return (
    <AppShell>
      ...
      <div className="grid grid-cols-12 gap-8">
        {/* APY Drift Analysis — Primary Card */}
        <section className="col-span-12 lg:col-span-8 space-y-6">
          <div className="bg-surface-container-lowest p-8 rounded-xl clinical-shadow">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="font-headline font-bold text-xl text-on-surface">APY Drift Analysis</h3>
            ...
            {selectedDrift ? (
              ...
              <p className="text-on-surface-variant text-sm">No drift data available for this vault</p>
            )}

            {/* APY Timeline visualization */}
            {selectedVault && (
              <ApyTimeline vault={selectedVault} />
            )}
          </div>
          ...
          <div className="bg-surface-container-lowest rounded-xl clinical-shadow">
            <div className="p-6 border-b border-surface-container">
              <h3 className="font-headline font-bold text-lg text-on-surface">All Vault Drift Scores</h3>
            ...
            <div className="divide-y divide-surface-container">
              {vaults
                ...
                .sort((a, b) => (computeApyDrift(b)?.absoluteDrift || 0) - (computeApyDrift(a)?.absoluteDrift || 0))
                .slice(0, 10)
                .map((vault, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 flex items-center justify-between cursor-pointer hover:bg-surface-container-low transition-colors ${
                        selectedVault?.address === vault.address ? 'bg-secondary-container/30' : ''
                      }`}
                    >
                      <div>
                        <p className="font-bold text-sm text-on-surface">{vault.name}</p>
                        <p className="text-xs text-on-surface-variant">{vault.protocol.name} · {vault.network}</p>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          ...
                          <p className={`font-bold text-sm ${
                            ...
                            {drift?.drift30d > 0 ? '+' : ''}{drift?.drift30d}%
                          </p>
                        </div>
                        <div className="text-right">
        ...
        <section className="col-span-12 lg:col-span-4 space-y-4">
          <div className="bg-surface-container-lowest p-6 rounded-xl clinical-shadow">
            <div className="flex items-center gap-3 mb-4">
              <span className="material-symbols-outlined text-on-primary-container">grid_view</span>
              <h3 className="font-headline font-bold text-lg text-on-surface">Volatility Heatmap</h3>
            </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-xl clinical-shadow space-y-4">
            ...
            {(() => {
              ...
              const avgDrift = withDrift.length
                ? withDrift.reduce((s, v) => s + (computeApyDrift(v)?.absoluteDrift || 0), 0) / withDrift.length
                : 0
              const stable = withDrift.filter(v => (computeApyDrift(v)?.absoluteDrift || 0) < 0.01).length
              const volatile = withDrift.filter(v => (computeApyDrift(v)?.absoluteDrift || 0) > 0.05).length

              return (
                <>
                  <StatRow label="Avg Drift (all vaults)" value={`${(avgDrift * 100).toFixed(2)}%`} />
                  <StatRow label="Stable vaults (<1% drift)" value={`${stable}`} positive />
                  <StatRow label="Volatile vaults (>5% drift)" value={`${volatile}`} negative={volatile > 0} />
...
function DriftMetric({ label, value, isActive, isPositive }) {
  return (
    <div className={`p-4 rounded-xl transition-all ${isActive ? 'bg-primary-container' : 'bg-surface-container-low'}`}>
      <p className={`text-[10px] uppercase tracking-widest font-bold mb-1 ${isActive ? 'text-on-primary-container' : 'text-on-surface-variant'}`}>
        {label}
      </p>
  ...
  return (
    <div className="flex justify-between items-center">
      ...
      <span className={`font-bold text-sm ${
        positive ? 'text-on-tertiary-container' : negative ? 'text-on-error-container' : 'text-on-surface'
      }`}>
        {value}
      </span>
    </div>
